...
//    Dan Geiger (Port to SMART Response XE)
//
// Contributors:
//          Brian O'Dell <megamemnon@megamemnon.com> (INPUT)
//    (full list tbd)
//          Dan Geiger for SMART device specific code
//          Larry Bank for Arduino Library
...
uint16_t eepos = 0; // keep track of eeprom position during read/write
int fileByte = 0; // track where we are in building a 256 byte packet to save
uint8_t memSend[256]; // Array holding the 256 byte packet to save
short int memloc; // external memory location (0-9)
uint32_t mempos = 0; // external memory address to write to

#define kRamFileIO (0)

#define kRamTones (0)

#define kRamSize  12288  //Set at 12k. Was (RAMEND - 1500 - kRamFileIO - kRamTones)
...
static int cROW = 0;
static int cCOL = 0;
static int dispROW = 0; // Track current line in display memory for scroll
...
#define SQUOTE  '\''
#define DQUOTE  '\"'
#define CTRLC 0xf4 // Special Function Button 5
#define CTRLH 0x08
#define CTRLS 0x13
#define CTRLX 0x18
...
static const unsigned char eepromamsg[]       PROGMEM = " EEProm bytes avail.";
...
static const unsigned char battmsg[]          PROGMEM = " mVolts";
  ...
  while (1)
  {
    ...
    switch (c)
    {
      ...
      case 0x02: // Backspace
        ...
        txtpos--;
        cCOL--;
        SRXEWriteString(cCOL * 12, dispROW * 16, " ", FONT_MEDIUM, 3, 0);
        break;
      default:
...
interperateAtTxtpos:
  ...
  switch (table_index)
  {
    ...
    case KW_STOP:
      ...
      goto execline;
    case KW_BYE: // Add functions to shut down the unit
      cls();
      SRXESleep();
      SRXEScrollArea(0, 144, 16);
      setBatStat();
...
list: // Added code to list by page and break on space bar
...
msave: // Save program in memory location 0 to 9
  {
    expression_error = 0;
    memloc = expression();
    if (memloc < 0 || memloc > 9  || expression_error) goto qwhat;

    clearMem(memloc);
    outStream = kStreamFile;
    mempos = memloc * 12288ul;
...
mload: // Load program in memory location 0 to 9
  expression_error = 0;
  memloc = expression();
  if (memloc < 0 || memloc > 9 || expression_error) goto qwhat;
...
void setup()
{
  ...
  SRXEFill(0);
  SRXEScrollArea(0, 144, 16); // Visible screen area
...
static unsigned char breakcheck(void)
{
  if (char kPress = SRXEGetKey())
    return kPress == CTRLC || 0xf1;
  return 0;
}
/***********************************************************/
...
static void outchar(unsigned char c)
{
  ...
  if ( outStream == kStreamFile ) {
    if (c == '\0') {
      memSend[fileByte] = c;
      SRXEFlashWritePage(mempos, memSend);
    }
    else {
      memSend[fileByte++] = c;
      if (fileByte == 256) {
  ...
  if ( outStream == kStreamSerial ) {
    switch (c) {
      ...
      default:
        ...
        SRXEWriteString(cCOL * 12, dispROW * 16, buf, FONT_MEDIUM, 3, 0);
        cCOL ++;
        if (cCOL > 31) {
          cCOL = 0 ;
          cROW ++;
...
void screenScroll() { // Scroll the display
  if (dispROW > 8) dispROW = 0;
  byte clearROW = dispROW + 1;
  if (clearROW > 8) clearROW = 0;
  SRXEWriteString(0, clearROW * 16, "                                ", FONT_MEDIUM, 3, 0);
  SRXEScroll(16);
  cROW = 7;
}
